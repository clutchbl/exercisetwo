...
def count_program(program_path, num_expected):
    """
    Count the number of processes that have the given program pathname.
    Error is asserted and the test is failed if the number of processes
    expected is not at least 0, and the number of processes discovered is
    not greater than or less than the number expected.

    Note: Looks at /proc/pid/exe, so program_path should be an absolute path.
    """
    assert num_expected >= 0, 'ERROR: invalid input'
    ...
    assert num_found >= num_expected, \
            'ERROR:  Fewer processes found than expected'
    assert num_found == num_expected, \
            'ERROR:  More processes found than expected'
